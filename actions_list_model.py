...
class ActionListModel(QAbstractListModel):
    ...
    def roleNames(self):
        ...
        default[ActionListModel.MetricsRole] = b"something"
        return default
    ...
    def data(self, index, role):
        if not self.db:
            ...
            ret = self.db[index.row()]["action"]
        elif role == ActionListModel.MetricsRole:
            ret = self.db[index.row()]["something"]
        else:
            ret = None
        return ret
    """
    ...
    def insertRows(self, row: int, count, new_actions, index=QModelIndex()):
        #Insert n rows (n = 1 + count)  at row

        self.beginInsertRows(QModelIndex(), row, row + count)

        # start database work
        for i, action in enumerate(new_actions):  # at least one row
            self.db.insert(
                row + i, {"something": action, "fill_the_gap": "fill_the_gap"}
            )
        # end database work
        self.endInsertRows()
        return True
